...
/-- The production policy as an edge selector. `hot` and `safe` are
fixed-target properties of the node; `low` is an intrinsic property of
the move, so neither the caller's window nor killer ordering enters. -/
def intrinsicEdgeSpend (G : QSGame) (hot safe : G.Pos → Nat → Bool)
    (low : G.Pos → Nat → G.Pos → Bool) : G.Pos → Nat → G.Pos → Nat :=
  fun p d m => intrinsicSpend (hot p d) (safe p d) (low p d m)

theorem intrinsicEdgeSpend_le_two (G : QSGame) (hot safe : G.Pos → Nat → Bool)
    (low : G.Pos → Nat → G.Pos → Bool) (p m : G.Pos) (d : Nat) :
    intrinsicEdgeSpend G hot safe low p d m ≤ 2 :=
...
theorem forcedMate_intrinsicValue (G : QSGame) (guard : G.Pos → Bool)
    (hot safe : G.Pos → Nat → Bool) (low : G.Pos → Nat → G.Pos → Bool)
    (hF : ValFloor G 192) {k : Nat} {p : G.Pos} (hFM : ForcedMate G k p) :
    ∀ D : Nat, 3 * k + 6 ≤ D →
      MATE_LOWER ≤ fuelValueD2 G guard 3 (intrinsicEdgeSpend G hot safe low) D p :=
  forcedMate_fuelValueD2 G guard 3 (intrinsicEdgeSpend G hot safe low) (by omega) hF hFM
...
theorem eventual_classification_intrinsic_finite (G : QSGame) (guard : G.Pos → Bool)
    ...
    ∀ D : Nat, 3 * N + 11 ≤ D →
      ((MATE_LOWER ≤ fuelValueD2 G guard 3 (intrinsicEdgeSpend G hot safe low) D p) ↔
        (∃ k, ForcedMate G k p)) ∧
      ((fuelValueD2 G guard 3 (intrinsicEdgeSpend G hot safe low) D p ≤ -MATE_LOWER) ↔
        (∃ k, ForcedlyMated G k p)) ∧
      ((¬ (∃ k, ForcedMate G k p)) → (¬ (∃ k, ForcedlyMated G k p)) →
        -MATE_LOWER < fuelValueD2 G guard 3 (intrinsicEdgeSpend G hot safe low) D p ∧
          fuelValueD2 G guard 3 (intrinsicEdgeSpend G hot safe low) D p < MATE_LOWER) := by
  intro D hD
  exact eventual_classification_fuel_finite G guard 3 (intrinsicEdgeSpend G hot safe low)

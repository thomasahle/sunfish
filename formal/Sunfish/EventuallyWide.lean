  ...
  | d + 1, p =>
    if G.eval p ≤ -MATE_LOWER then -MATE_UPPER
      ...
      foldMax (fun m => -(fuelValueD2t G guard C spend d m)) (tailList G (d + 1) p)
        (if guard p = true ∧ 2 < d + 1 then
          (if -(fuelValueD2t G guard C spend (d + 1 - 4) (G.pass p)) < MATE_LOWER then
            max LOSS (-(fuelValueD2t G guard C spend (d + 1 - 4) (G.pass p)))
          else LOSS)
        else LOSS)
    else
...
theorem fuelTermD2t_ge_LOSS (G : QSGame) (guard : G.Pos → Bool)
  ...
  · rw [if_pos h1]
    by_cases h2 : -(fuelValueD2t G guard C spend (d + 1 - 4) (G.pass p)) < MATE_LOWER
    · rw [if_pos h2]; omega
    · rw [if_neg h2]; omega
  · rw [if_neg h1]; omega
          ...
          | false =>
            ...
            · rw [fuelValueD2t_of_fold_sub G guard C spend d p hkg hcap hai (by omega)]
              ...
              have hTu : fuelTermD2t G guard C spend d p ≤ MATE_UPPER := by
                ...
                by_cases h1 : guard p = true ∧ 2 < d + 1
                · rw [if_pos h1]
                  have := ih (d + 1 - 4) (by omega) (G.pass p)
                  by_cases h2 : -(fuelValueD2t G guard C spend (d + 1 - 4) (G.pass p))
                      < MATE_LOWER
                  · rw [if_pos h2]; omega
                  · rw [if_neg h2]; omega
                · rw [if_neg h1]; omega

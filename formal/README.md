...
From depth 6 on the pass is not a score candidate at all. Two fixed targets
...
if depth >= 6 and abs(pos.score) < 500 and any(c in pos.board for c in "RBNQ"):
    ...
    d -= -self.bound(nullpos, 1 - hot_target, depth - 3) >= hot_target
    safe = d < depth or -self.bound(nullpos, 1 - safe_target, depth - 3) >= safe_target

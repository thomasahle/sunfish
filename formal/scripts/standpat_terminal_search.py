#!/usr/bin/env python3
"""Counterexample search for StandPatAtTerminal (formal/Sunfish/Stalemate.lean).

The Lean side leaves StandPatAtTerminal as a HYPOTHESIS: at a depth-0
correction-terminal position the stand-pat pseudo-option (sunfish.py
lines 386-387, `yield None, pos.score`) must not exceed the terminal
value.  Its violation on a real board is the crisp crossing shape of
...
* EVERY pseudo-legal `pos.gen_moves()` has `pos.value(m) >= 40`
  (`val_lower` at depth 0), so the correction gate
  `all(pos.value(m) >= val_lower ...)` (lines 484-485) passes.

At such a position a low-gamma depth-0 probe cuts off on the stand-pat
and stores `lower = pos.score > terminal`, while a high-gamma probe runs
the loop dry (`best_real` stays at the sentinel), corrects, and stores
`upper = terminal`: a transposition-table crossing.

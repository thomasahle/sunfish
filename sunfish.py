...
def main():
    ...
    while True:
        ...
        while move not in hist[-1].gen_moves():
            usr_inpt = input('Your move: ')

            if usr_inpt == 'q' or usr_inpt == "quit":
                sys.exit()

            if usr_inpt == 'h' or usr_inpt == "help":
                print("\nHELP:")
                print("Rows are annotated from bottom to top using numbers 1 to 8.")
                print("Columns are annotated from left to right using alphabets a to h.")
                print("If you wish to move your piece from column b row 2 to column b row 3, then enter the move as b2b3\n")
                continue

            match = re.match('([a-h][1-8])'*2, usr_inpt)
            if match:

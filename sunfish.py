...
class Searcher:
    ...
    def bound(self, pos, gamma, depth, root=False):
        ...
        for move in moves():
            ...
            # result (None), so no search and no legality witness - and stop,
            # since the sorted stream caps lower from here on. Only the
            # out-of-order killer may not stop the fold, and it is the one
            # candidate that IS the killer object; the sorted stream re-yields
            # an equal move built fresh by gen_moves.

...
NULL_MIN_DEPTH NULL_LIMIT NULL_CUT_RED NULL_RED IID_MIN_DEPTH IID_RED FUT_MAX FUT_CAP
...
the hot node's extra depth cost). `FUT_CAP` selects no depth-2/3 cap, the
current quiet-move cap, or the simpler negative-`value()` cap. `VALUE_N VALUE_B

...
nn = [np.frombuffer(ar, dtype=np.int8) / 127. for ar in model['ars']]
...
# It could also be fun to use a super small CNN, but I couldn't make it work
# as well as NNUE.
# def conv(x, kernel):
#     h, w, c = x.shape
#     windows = np.lib.stride_tricks.sliding_window_view(x, (2,2), (0,1))
#     # The kernel should be (c, h, w) flattened
#     return windows.reshape(h-1, w-1, c*4) @ kernel.T
...
# print(f'{pst["P"][A1]=}')
# print('Initial value', value(initial, verbose=True))
...
class Position(namedtuple('Position', 'board score wf bf wc bc ep kp')):
    ...
    def rotate(self, nullmove=False):
        ...
        pos = Position(self.board[::-1].swapcase(), 0, self.bf, self.wf, self.bc, self.wc,
    ...
    def move(self, move):
        def put(pos, i, p):
            ...
            if p.isupper(): wf += pst[p][i]
            if p.islower(): bf += pst[p.upper()][119-i] # Presumably this never happens?
            q = pos.board[i]
            if q.isupper(): wf -= pst[q][i]
            if q.islower(): bf -= pst[q.upper()][119-i]
    ...
    def is_capture(self, move):
        ...
        return self.board[move.j] != '.' \
                or move.j == self.ep \
                or abs(move.j-self.kp) < 2 \
                or self.board[move.i] == 'P' and A8 <= move.j <= H8
...
class Searcher:
    ...
    def bound(self, pos, gamma, depth, root=True):
        ...
        #     print('Stack is getting very long...', file=sys.stderr)
        #     print(pos.board, file=sys.stderr)
        #     print(pos.score, file=sys.stderr)
        ...
        # Table part 2
        self.tp_score[pos.hash(), depth, root] = Entry(best, entry.upper) \
                           if best >= gamma else Entry(entry.lower, best)
    ...
    def search(self, history):
        ...
        for depth in range(1, 1000):
            ...
            while lower < upper - EVAL_ROUGHNESS:
                ...
                if score < gamma:
                    upper = score
                    yield depth, self.tp_move.get(pos.hash()), score, 'upperbound'
    ...
    while True:
        match input().split():
            case ['uci']:
                ...
                print('option name QS_LIMIT type spin')
                print('option name EVAL_ROUGHNESS type spin')
            ...
            case ['go', *args]:
                if args and args[0] == 'movetime':
                    think = int(args[1]) / 1000
                elif args and args[0] == 'wtime':
                    _, wtime, _, btime, _, winc, _, binc = args

...
## 2026-08-16 — SETTLER VERDICT: AMBIGUOUS, and the third branch is what saved the reading

Gates first, both passed: **0 forfeits, 0 illegal, 300/300 `normal`**. Arms
sha-verified box-side (prespeed **3405 B** `5a207fdf…` vs speed **3376 B**
`a997b137…`), venue gate passed with 76 free cores, srand 20260824, 2 h 26 m.

### The result, from the speed arm's perspective

| | |
|---|---|
| **Elo (speed vs pre-speed)** | **+23.20 ± 31.12** → **[−7.92, +54.32]** |
| nElo | +29.47 ± 39.32 |
| score | **53.33%** — 99 W / 79 L / 122 D |
| pentanomial | **[10, 35, 49, 37, 19]** over 150 pairs; PairsRatio 1.24 |
| draw ratio | **32.67%**; **LOS 92.91%** |

### The three pre-registered readings, applied exactly

| # | rule | fires? |
|---|---|---|
| 1 | **HOLDS** — interval excludes 0 **and** point ≥ +20 | **NO** — point is +23.20 ✓ but the interval includes 0 (lo = −7.92) ✗ |
| 2 | **DISCOUNTED** — upper bound < +33 | **NO** — upper is +54.32 |
| 3 | **AMBIGUOUS** — anything else | **YES** |

> **VERDICT: AMBIGUOUS.** The registered response is **more games on this
> pairing, never narration.**

**This is the case the third branch was written for.** A point estimate of
**+23** sits squarely in "the +20s-30s", and under the original two-reading
scheme it would have been announced as *RELATION HOLDS* — a confirmation of
the +33 projection. It is not one. The interval contains **everything the
match was supposed to choose between**:

| hypothesis | value | inside [−7.92, +54.32]? |
|---|---|---|
| no effect at all | 0 | **yes** |
| meter-3 residual lean | +3.8 | **yes** |
| the projection | +33 | **yes** |
| the model, 1.46 × 25.3% | +36.9 | **yes** |

A test that cannot exclude any of its own hypotheses has not arbitrated;
it has only narrowed the range a little. **LOS 92.91%** says the speed arm
is *probably* ahead — suggestive, and below the bar this ledger uses.

### The implied nps→Elo slope — the number the axis actually needs

Dividing the observed Elo by the stack's measured **+25.3%** nps
(quiet-venue, n=8):

> **empirical 0.92 ± 1.23 Elo per 1% nps**, against the modeled **1.46**.

**Both the model and zero sit inside that interval.** The axis the
remaining ~200 Elo leans on is **still uncalibrated** — this match moved
the error bar, not the answer.

### Consequence for Design B and every queued speed figure

**Design B's ≈+25-30 is NOT discounted** — the DISCOUNTED branch did not
fire, and applying its arithmetic anyway would be exactly the narration the
registration forbids. **But it is not confirmed either.** The honest status
of every speed projection in the queue is unchanged from before this match:
**an unvalidated model estimate**, and it should be quoted with that label
rather than as an expected gain.

### My own power estimate was optimistic, and that is why the branch existed

I registered **±24.5**; the match delivered **±31.12** — 27% wider, because
draws ran **32.67%** against the ~25% my slope calculation implicitly
assumed. Had I trusted the tighter figure and skipped the ambiguous branch,
this result would have been read as a confirmation. **The lesson is to
register the branch you expect not to need.**

### What finishing the job costs — read ONCE, at a fixed N

| goal | total N | additional games |
|---|---|---|
| establish the effect is above zero | ~540 | **~240 more** |
| separate +33 from 0 | ~1067 | **~767 more** |

**Sequential-testing hazard, stated so it is not walked into:** adding games
to this pairing and re-reading until it clears is not a fixed-N measurement
and would inflate the false-positive rate. Any continuation must
**pre-register a fixed total and read once** — the same rule that made
meter 3 and this settler trustworthy.

**Recommendation:** if the axis matters enough to calibrate — and with ~200
Elo of the goal leaning on it, it does — the ~540-game continuation is the
cheap version and answers "did it help at all". The ~1067-game version is
what actually calibrates the slope, and at 2 h 26 m per 300 games on the box
that is roughly 8-9 hours.
...
## 2026-08-16 — THE +400 PROGRESS METER: +244.47 ± 39.23 at 60+1, clean, fixed-N, 300 games

The number the standing goal is measured against. It took a voided laptop
run, a venue relocation, and a registration that had to refuse its own
first attempt — and it is the first time this campaign has had a **clean,
fixed-N, real-clock** magnitude for the 4k entry against classic.

### The gates were read first, in the registered order

The registration said forfeits before Elo, and that order was followed:

| gate | result |
|---|---|
| **time forfeits** (VOID condition) | **0** |
| **illegal moves** (STOP condition) | **0** |
| terminations | **300 / 300 `normal`** |

**Only after both gates passed was the Elo computed.** On the box's own
650+ clean games it was likely to pass — but it was checked, not assumed.

### The result

| | |
|---|---|
| **Elo** | **+244.47 ± 39.23** (95%, pentanomial) → **[+205.24, +283.70]** |
| nElo | **+337.26 ± 39.32** |
| score | **80.33%** — 207 W / 25 L / 68 D |
| pentanomial | **[1, 6, 22, 52, 69]** over **150 complete pairs** |
| PairsRatio | **17.29** |
| draw ratio | 14.67%; LOS 100.00% |
| venue | bench box, **concurrency 8 (declared)**, srand 20260822, 2 h 05 m, load 25.67 at launch |
| arms | entry **3405 B** `5a207fdf9cf05f2e…` vs classic **3246 B** `7bdbd6054f70622a…` — **transferred and sha-verified, not rebuilt**, boot-smoked on the box before games |
| label | **pre-#205-classic** (classic pinned at master `2784dbf`) |

Independent round-paired recompute returns classic **−244.47 ± 39.23**,
the exact mirror of fastchess. Not one game dropped.

### What it does to B's glimpse: CONFIRMS and TIGHTENS

| | estimate | 95% interval |
|---|---|---|
| B (N=60, clean) | +325.17 ± 134.54 | [+190.63, +459.71] |
| **meter (N=300, clean)** | **+244.47 ± 39.23** | **[+205.24, +283.70]** |

The meter's point estimate falls **inside** B's interval, its interval is
**3.4× narrower**, and the two differ by only **z = 1.13**. So B is
**settled lower within its own error bar, not overturned** — exactly what
a five-fold increase in games should do to a wide glimpse. B was right
about the direction and the order of magnitude; the meter supplies the
precision.
...
favourable end of the interval.

### The two instruments together, which is the real finding

| instrument | result | what it isolates |
|---|---|---|
| fixed-node 20000, N=400 | **−1.74 ± 27.93** | search + eval per node → **parity with classic** |
| **clean 60+1, N=300** | **+244.47 ± 39.23** | shipped form on a clock → **entry dominates** |

**The entry's entire advantage over classic is speed and time
management** — this is now measured on both sides rather than inferred
from one. The distilled PSTs are worth about what classic's hand-tuned
tables are worth *per node*; the artifact wins because it searches more
nodes per second and spends them better. Consistent with this ledger's
"SPEED IS ELO" finding, and it says where the remaining 155 Elo will not
come from: another eval fit.

### Provenance chain

- Run 1 (laptop) **VOID** — 9 forfeits, all the entry, all venue-driven
  (forfeited games at median 200% foreign CPU vs 36% for the 291 normal).
  It cost time and nothing else.
- **Relocation branch (b)** fired on evidence, moving the venue to the box.
- Arms are **byte-identical** across stage 1, replications A and B, run 1
...
**IT LANDED (2026-08-16): +244.47 ± 39.23**, 300/300, zero forfeits — see
the verdict entry above. B's +325.17 ± 134.54 is confirmed and tightened
rather than overturned, and stage 1's fixed-node −1.74 ± 27.93 stands
untouched.
...
**SUPERSEDED 2026-08-16 by the relocated meter, +244.47 ± 39.23 over 300
games** — which confirms B's direction and magnitude and tightens its
interval 3.4×. B was the only clean timed number when this was written. Stage 1's fixed-node **−1.74 ± 27.93** is
untouched — a fixed-node match reads no clock and is co-runnable by
design.

---

## 2026-08-15 — A NEAR-MISS ON THE PRIMARY MEASUREMENT: stale lock metadata, and two liveness checks that both misfire

Nothing was damaged and the meter never stopped. This is recorded because
the *cause* was this lane's, and because the two obvious ways to ask "is a
match running?" are both wrong in opposite directions.

### What happened

Another lane read the laptop screen lock. Its purpose line still said
**"forfeit replications A and B"** — the previous hold — and the body
carried **A and B COMPLETE** markers. Reasonably, that lane concluded the
lock was awaiting cleanup, identified the meter's `fastchess` as "one
lingering process, the thing to reap", and flagged it. The barrier held.

**A live N=300 match was playing under that lock at the time.**

> **RULE: a re-taken lock must have its PURPOSE TEXT REWRITTEN AT TAKE
> TIME.** Stale purpose text on a live lock is how another lane politely
> kills your match. The holder is not enough; the *purpose*, the *ETA* and
> the *release condition* must all describe the CURRENT hold.

### Both liveness checks misfire, and I hit both today

| check | verdict | evidence |
|---|---|---|
| `ps -Ao comm \| grep -x fastchess` | **FALSE NEGATIVE** | `comm` holds the **full path**, so the bare name never matches. It returned **0 while the meter was live and writing games.** |
| `pgrep -f fastchess` | **FALSE POSITIVE** | Matches the *reader's own* shell wrappers containing the word. At 15:08 it reported a match running when none was. |

**The false negative is the dangerous one** — it is exactly the reading
that authorizes reaping a live match, and it is the reading the other lane
would have gotten.

**The tested replacement**, now written into the lock itself:

```
ps -Ao pid,args | grep "opponents/bin/fastchess-mac-arm64/fastchess" \
                | grep "arena-meas/meter.pgn" | grep -v grep
```

It requires **both** the real binary path **and** the pgnout that identifies
*this* match, and excludes itself. Verified: exactly one pid, no match
against this lane's own shells.

### Two smaller things caught in the same pass

- **The first rewrite computed a negative rate** (−0.37 games/min, ETA
  05:44Z) from a hand-typed start epoch. Caught before it shipped; the
  lock's ETA is now derived from the PGN's own `GameStartTime` /
  `GameEndTime` tags. **A wrong ETA on a lock is the same defect class as
  a stale purpose** — metadata that invites a bad decision.
- `run_meter.sh` now writes the full guard text **at take time**, so a
  future re-take cannot inherit a stale purpose the way this one did.

**Meter unaffected throughout: 70/300, 0 forfeits, 0 illegal** — and its
score remains deliberately unread, per the registered no-early-stop rule.
...
## 2026-08-15 — REPLICATION B: the clean floor is ZERO, the forfeit investigation CLOSES, and we get the first clean real-clock number

B ran its full registered length on a quiet machine — the arm the whole
study was pointing at.

| | |
|---|---|
| form | 60+1, **fixed N = 60**, **concurrency 1**, packed arms, adjudication off, srand 20260819 |
| result | **60 / 60 games** |
| **illegal** | **0** |
| **time forfeits** | **0** |
| terminations | **60 / 60 `normal`** |
| venue during B | foreign CPU **median 12%**, p90 105%, max 459% |
| Elo | entry **+325.17 ± 134.54** (classic scored 13.33%) |

### The floor, and the decomposition it makes possible

| | contended (A) | clean (B) |
|---|---|---|
| moves past the entry's own deadline | 13.74% | **9.88%** |
| **worst self-overrun** | **+22 435 ms** | **+557 ms** |
| minimum flag margin | −1030 ms (unreliable, see A) | **+503 ms** |
| forfeits | 25 | **0** |

**The rate barely moves; the TAIL collapses 40×.** That splits the defect
cleanly in two, and only one half is ours:

- **Intrinsic and harmless.** The entry runs past its own deadline on
  roughly one move in ten *in both venues*. On a quiet machine the worst
  case is **557 ms**, against a smallest observed flag margin of
  **+503 ms** — it never threatens a flag.
- **Venue, and decisive.** Foreign core saturation multiplies that tail
  into **seconds**, which is what flags. The engine cannot defend against
  it because a descheduled process cannot execute its own deadline check.

**Pooled clean-venue evidence: 0 forfeits in 74 games** (B's 60 plus A's 14
clean-classified), giving an exact one-sided 95% bound of **< 3.97%**.

### THE FORFEIT INVESTIGATION IS CLOSED

- **Mechanism:** CPU starvation of a niced process under foreign core
  saturation. Established by dose-response, by the 40× tail collapse, by
  651 clean games on an uncontended box, and by `classic` flagging once
  too (an entry-only defect cannot do that).
- **Clean floor:** zero forfeits, < 3.97%.
- **Holdback fix — DISPOSITION: registered, NOT run, and it stays that
  way.** It would trim milliseconds from a quantity that is harmless at
  ≤ 557 ms, and it is powerless against seconds of scheduler starvation.
  **Revisit only if a forfeit ever appears at zero foreign load** — that
  observation, and nothing else, reopens it.
- **The `timemargin` sweep** likewise stays registered-not-run: with a
  clean floor of zero there is nothing for it to measure.
- **What actually prevents recurrence** is the venue-exclusivity rule, not
  an engine change.

### The timed number: a GLIMPSE, with every caveat the registration promised

**+325.17 ± 134.54.** What it is not:

- **Not the meter.** N=60 buys ±135. The proper measurement is registered
  separately at N=300.
- **Packed-vs-packed**, so each side carries its own shipped time manager
  — entry `_pooltm`, classic **min40-4** (the manager #196 landed at +147
  over its predecessor). This is shipped-form, not engine+eval-only.
- **One match, one srand, one venue.**

What it nonetheless means, stated as honestly as the caveats:

- It is the **first clean real-clock entry-vs-classic measurement this
  lane has ever produced.** Every previous timed number was either a
  different engine (the +187 was 256kb8@100M), a contaminated venue, or a
  driver-vehicle match that bypassed the shipped time manager.
- **The league plays real clocks.** Fixed nodes is an instrument, not the
  competition.
- **Even the interval's low end, ~+190, is a large gap.**

### Both numbers are true, and together they say something specific

| instrument | result | what it isolates |
|---|---|---|
| fixed-node 20000, N=400 | **−1.74 ± 27.93** | search + eval per node → **parity with classic** |
| clean 60+1, N=60 | **+325.17 ± 134.54** | shipped form on a clock → **entry dominates** |

The entry's advantage is **not** per-node quality; it is **nps and time
management**, exactly as this ledger's "SPEED IS ELO" finding predicts and
as stage 1 already implied. The distilled PSTs are worth about what
classic's hand-tuned tables are worth per node — and the artifact wins
anyway once a clock exists.

---

## 2026-08-15 — REPLICATION A: VERDICT. The forfeits are a venue effect, and the arm that measured it was itself contaminated

A ran its full registered length and was never stopped for forfeits —
those were the measurement. Only an illegal move would have stopped it,
and there were none.

| | |
|---|---|
| form | 60+1, **fixed N = 100**, packed arms, adjudication off, srand 20260818, concurrency 4 |
| result | **100 / 100 games** |
| **illegal moves** | **0** |
| **time forfeits** | **25** — **24 the entry, 1 classic** |
| terminations | 75 normal, 25 time forfeit |
| Elo | entry **+96.19 ± 66.86** — **VENUE-DAMAGED, do not quote** |

### The dose-response, final

87 of 100 games have sampler coverage (13 predate the sampler and are
excluded, never folded in):

| foreign CPU during the game | games | forfeits | rate |
|---|---|---|---|
| **QUIET** (< 40%) | 9 | **0** | **0.0%** |
| **LIGHT** (40 – 100%) | 5 | **0** | **0.0%** |
| **BUSY** (100 – 250%) | 45 | 14 | **31.1%** |
| **SATURATED** (> 250%) | 28 | 11 | **39.3%** |
| clean total (≤ 100%) | **14** | **0** | **0.0%** |
| contended total (> 100%) | 73 | 25 | 34.2% |

**Fourteen clean games, zero forfeits.** Exact one-sided 95% bound on the
clean-venue rate: **< 18.9%** — an improvement on the interim 34.8%, and
still not a floor. That floor is B's job.

**`classic` flagged once as well.** A defect intrinsic to the entry's time
manager cannot produce that; **CPU starvation, which does not care which
process it descheduled, predicts it.** One event is not proof, but it
points the same way as everything else.

### Why the Elo must not be used

**+96.19 ± 66.86 to the entry is not the timed +400 number and must never
be quoted as one.** Twenty-five games — a quarter of the match — were
decided by a flag rather than by chess, 24 of them against the entry. The
true timed gap is *better* than +96 for the entry, by an unknown amount.
The honest position is that **this lane still has no clean real-clock
measurement of entry vs classic**; stage 1's fixed-node −1.74 ± 27.93
stands, and the timed number remains unmeasured.

### A was not the arm it was registered as

Foreign CPU across A: median **193%**, p90 **411%**, max **903%**. A was
registered to reproduce the void's conditions and instead ran on a
*more* contended machine, so its 25% headline rate is **not** comparable
to the void's 4.9%. What A delivers is the dose-response and the
mechanism, not a rate for any venue.

### Correction to my own quantization claim

I claimed from 7 samples that the arbiter's overrun was a fixed ~100 ms
quantum. With 25 more: **23 of 25 are 100-102 ms, but two are 17 ms and
30 ms.** So it is *predominantly* a ~100 ms detection granularity, not
exclusively one. The conclusion is unchanged — the reported magnitude
still tells us nothing about an engine whose measured self-overrun reaches
**22 435 ms** — but the claim as first stated was too strong.

### Reconstruction limit, recorded

The reconstructed flag margin shows 17 moves below zero while the arbiter
reports 25 forfeits. The two do not have to agree: the reconstruction
never sees the flagging move (no move is produced), and small per-move
clock errors accumulate over 40+ moves. **Self-overrun is robust to this**
(it depends on the reconstructed clock only through the slowly-varying
`think()`), but the absolute flag margin near zero is not — forfeits come
from the arbiter, always.

**B is now waiting for a quiet venue** and is the only arm that will
measure the clean floor.
...
### 4. PRE-REGISTERED, and LAUNCHED on the coordinator's ruling

Registered before any game, in the 8Mv screen's exact form:

| | |
|---|---|
| screen | fixed-node SPRT, elo0=0 elo1=10, alpha=beta=0.05, nodes 20000, cap 1000 games, srand 20260814 |
| book | `book3k.pgn`, order=random — the laptop's registered book (`openings_2k` is the box's file, and the packed entry parses only `position startpos moves …`, so PGN not EPD) |
| arm | ml2 entry, `21_phase_ml2_dense` payload at shift 2, packed **3884 B** |
...
| gate | zero illegal moves; one ends the run and voids it |
| arena | `scratchpad/arena-ml2export/`, laptop, `nice -n 5`, concurrency 10 |

**Why the shift-2 arm is screenable, ruled by the coordinator and written
in before game 1.** The shift was picked by minimising \|Δcp\| against the
trained float model on training-side probe positions — **val-side model
selection**, the same class of choice as picking `l1=.0005` by family
val. No play outcome informed it, so the play bar is uncontaminated and
the registered bar stands unchanged. (My earlier reading — that this was
a design change under `5457f27`'s precedent — was over-cautious: that
precedent covers changes justified by *play* results.)

**Arm/base asymmetry, disclosed in advance:** the arm predates the
`pooltm` landing and the base carries it. At **fixed nodes the time
manager is clock-dead on both sides**, so the screen stays a pure eval
question. If it passes, the merged artifact (ml2 + `pooltm`) is built and
**measured** as a separate follow-up (≈3941 B expected — the expectation
is worth nothing, the measurement governs) before any timed leg.

The other half of the coordinator's ruling runs in parallel: the **`u2`
retrain** — `u2` snapped on the certified grid inside forward, STE at the
export scale, exactly as the ternary weights already are — is queued on
the box (TRAINQUEUE.md carries both entries and the queue-idle gap).

The **subsumption ablation stays a registered follow-up obligation, not
part of this screen**: net-vs-net+pend, so the ml2 arm must be shown to
subsume the hand-built terms it is meant to replace rather than merely to
